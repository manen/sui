...
#[derive(Clone, Debug)]
/// self.size() is self.layable.size(), the centering only happens on self.render()
pub struct CenterY<L: Layable> {
	layable: L,
}
impl<L: Layable> CenterY<L> {
	pub fn new(layable: L) -> Self {
		Self { layable }
	}

	fn l_det(&self, det: Details, scale: f32) -> Details {
		let (_, l_h) = self.layable.size();
		let l_h = l_h as f32 * scale;

		let y_offset = (det.ah as f32 / 2.0 - l_h / 2.0) as i32;
		let l_det = crate::Details {
			x: det.x,
			y: det.y + y_offset,
			aw: det.aw,
...
impl<L: Layable> Layable for CenterY<L> {
	fn size(&self) -> (i32, i32) {
		self.layable.size()
	}
	fn render(&self, d: &mut crate::Handle, det: crate::Details, scale: f32) {
...
#[derive(Clone, Debug)]
/// self.size() is self.layable.size(), the centering only happens on self.render()
pub struct CenterX<L: Layable> {
	layable: L,
}
impl<L: Layable> CenterX<L> {
	pub fn new(layable: L) -> Self {
		Self { layable }
	}

	fn l_det(&self, det: Details, scale: f32) -> Details {
		let (l_w, _) = self.layable.size();
		let l_w = l_w as f32 * scale;

		let x_offset = (det.aw as f32 / 2.0 - l_w / 2.0) as i32;
		let l_det = crate::Details {
			x: det.x + x_offset,
			y: det.y,
			aw: det.aw,
...
impl<L: Layable> Layable for CenterX<L> {
	fn size(&self) -> (i32, i32) {
		self.layable.size()
	}
	fn render(&self, d: &mut crate::Handle, det: crate::Details, scale: f32) {
